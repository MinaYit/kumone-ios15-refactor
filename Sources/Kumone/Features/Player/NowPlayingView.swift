...
private struct MinimalTrackInfoRow: View {
    ...
    private var dismissButton: some View {
        // Match the immersive header: this is a native state-driven
        // dismissal, not an optional callback that can silently do nothing.
        ...
        .accessibilityIdentifier("minimalCloseButton")
    }
...
private struct MinimalTransportControls: View {
    ...
    private var queueSheet: some View {
        if #available(iOS 16.4, *) {
            ...
            // iOS 15 presents the same queue through the system full-height
            // sheet; no unavailable detent API is invoked.
            MinimalQueueSheet(backdrop: backdrop)
        }
    }

    private var playbackModeButton: some View {
        // 与参考图右下角一致：无论是否为 FM 队列，均保留循环模式入口，
        // 使五项控制在视觉和触摸上保持稳定。
        ...
        .accessibilityIdentifier("minimalPlaybackModeButton")
    }
